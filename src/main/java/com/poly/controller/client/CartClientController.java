...
public class CartClientController {
	...
	public String addToCart(HttpServletRequest request, HttpSession session,
		...
		ProductDTO productDTO = productService.findById(productId); // lay thong tin cua san pham khi nguoi dung chon
																	// mua
		float unitPrice = productDTO.getPrice()
				- Math.round((productDTO.getPrice() * productDTO.getSaleDTO().getSalePercent() / 100));

		Object object = session.getAttribute("cart"); // lay danh sach san pham trong gio hang tren session

		if (object == null) { // neu gio hang trong => them san pham vao gio hang va tao session moi chua gio
								// hang
			...
			} else { // neu san pham da co trong gio hang => tang so luong san pham do trong gio hang
						// len
